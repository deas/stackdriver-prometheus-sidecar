...
import (
	...
	ttest "github.com/Stackdriver/stackdriver-prometheus-sidecar/test"
...
func TestStoreErrorHandling(t *testing.T) {
	...
	tests := []struct {
		...
		{
			status: status.New(codes.InvalidArgument, longErrMessage), /*mustStatusWithDetails(status.New(codes.InvalidArgument, longErrMessage),
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			ctx := ttest.WithClient(ttest.WithFakeAuth(context.Background()))
			// assert := ttest.Assert(ctx, t)
			// data :=
			...
			*/
			...
			/*
				ctx := context.WithValue(ttest.WithClient(ttest.WithFakeAuth(context.Background())),
			...
			*/
			...
			/*
				...
				metricsaApi := datadogV2.NewMetricsApi(apiClient)
			*/
			// datadog.NewConfiguration()
			// datadog.NewAPIClient()
			// serverURL, err := url.Parse("https://invalid" /* + listener.Addr().String() */ + "?auth=false")
			// if err != nil {
			//	t.Fatal(err)
			// }
			...
			c := NewClient(configuration, time.Second, log.NewLogfmtLogger(logBuffer))
			/*
				c := NewClient(&ClientConfig{
					URL:     serverURL,
					Timeout: time.Second,
...
func TestEmptyRequest(t *testing.T) {
	...
	*/
	c := NewClient(&datadog.Configuration{}, time.Second, nil)

	/* c := NewClient(&ClientConfig{
		URL:     serverURL,
		Timeout: time.Second,
	})*/
	if err := c.Store([]datadogV2.MetricSeries{{}}); err != nil { // &monitoring.CreateTimeSeriesRequest{}
		t.Fatal(err)
	}
}
